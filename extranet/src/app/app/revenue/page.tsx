...
const RevenuePage: React.FC = () => {
  ...
  const filteredRevenueDetails = revenueData?.revenueDetails
    ? revenueData.revenueDetails.filter((property) => {
        ...
        const matchesMinAmount = filterCriteria.minAmount
          ? property.totalAmount >= Number(filterCriteria.minAmount)
          : true;
        const matchesMaxAmount = filterCriteria.maxAmount
          ? property.totalAmount <= Number(filterCriteria.maxAmount)
          : true;
  ...
  const fetchPropertyData = (propertyId: string, propertyName: string) => {
    router.push(
      `/app/revenue/revenueDetails?revenueId=${propertyId}&propertyName=${encodeURIComponent(propertyName)}`
    );
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {isLoading ? (
          <div className="text-center text-gray-600 dark:text-gray-400">
            <p className="text-lg font-medium">Loading revenue data...</p>
          </div>
        ) : !revenueData ? (
          <div className="text-center text-gray-600 dark:text-gray-400">
                ...
                {topProperty ? (
                  <>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-sm">
                      Revenue: ${topProperty.revenue.toLocaleString()}
                    </p>
                  </>
                ) : (
                ...
                <div className="overflow-x-auto">
                  {filteredRevenueDetails?.length > 0 ? (
                    <table className="w-full">
                      ...
                      <tbody>
                        {filteredRevenueDetails.map((property) => (
                          <tr
                            key={property._id}
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {topProperties.length > 0 ? (
                  topProperties.map((property, index) => (
                    <div
                      key={index}
                      className="p-6 bg-gray-100/80 dark:bg-gray-900/40 backdrop-blur-xl rounded-2xl border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 transition-all duration-300 hover:shadow-[0_0_15px_rgba(168,85,247,0.2)] hover:-translate-y-0.5"
                    >
                      ...
                      <div className="h-2 bg-gray-300 dark:bg-gray-700 rounded-full">
                        <div
                          ...
                          style={{ width: `${(property.revenue / 100000) * 100}%` }}
                        ></div>
                      </div>
