...
const PropertyRevenuePage: React.FC = () => {
    ...
    // if (isLoading) return <Loader />; // Use the Loader component here
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="text-center mb-6 animate-[slideDown_0.5s_ease-out]">
                    <h3 className="text-3xl font-medium bg-gradient-to-r from-blue-500 to-blue-700 dark:from-blue-400 dark:via-purple-400 dark:to-blue-400 bg-clip-text text-transparent">
                        Revenue Details for {propertyName}
                    </h3>
                    <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm">Track and analyze revenue performance for this property</p>
                </div>

                {/* Filter Section */}
                <div className="mb-6 flex flex-wrap md:flex-nowrap items-center gap-4">
                    <div className="flex flex-col w-full md:w-[30%]">
                        <label htmlFor="startDate" className="text-gray-600 dark:text-gray-400 text-xs mb-1">Check-In Date</label>
                        <input
                            type="date"
                            name="startDate"
                    ...
                    <div className="flex flex-col w-full md:w-[30%]">
                        <label htmlFor="endDate" className="text-gray-600 dark:text-gray-400 text-xs mb-1">Check-Out Date</label>
                        <input
                            type="date"
                            name="endDate"
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-gray-200 dark:border-gray-800">
                                        <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-gray-400">User</th>
                                        <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-gray-400">Booked On</th>
                                ...
                                <tbody>
                                    {filteredBookings.map((booking) => (
                                        <tr key={booking._id}
                                            ...
                                            <td className="px-6 py-4 text-gray-800 dark:text-gray-200">{new Date(booking.booking_dates).toLocaleDateString("en-GB")}</td>
                                            ...
                                            <td className="px-6 py-4 text-gray-800 dark:text-gray-200">{new Date(booking.checkInDate).toLocaleDateString("en-GB")}</td>
                                            <td className="px-6 py-4 text-gray-800 dark:text-gray-200">{new Date(booking.checkOutDate).toLocaleDateString("en-GB")}</td>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md transition-opacity duration-300 animate-fadeIn">
                        <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-2xl text-gray-800 dark:text-gray-200 w-[90%] max-w-md animate-slideUp">
                            ...
                            <div className="space-y-3 text-gray-700 dark:text-gray-300">
                                <p><span className="font-medium text-gray-900 dark:text-gray-100">🏨 Room:</span> {selectedBooking.room.room_name}</p>
                                <p><span className="font-medium text-gray-900 dark:text-gray-100">📌 Type:</span> {selectedBooking.room.room_type}</p>
                                <p>
