...
export const TaxGroups = ({
    ...
    const handleToggleActive = async (groupId: string, currentStatus: boolean) => {
        ...
        try {
            ...
            if (err.response) {
                toast.error(`Failed to toggle status: ${err.response.data?.error || err.response.data?.message || "Unknown error"}`);
            } else {
                toast.error("Network error");
            }
        } finally {
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        <h5 className="text-md font-medium text-gray-900 truncate">{group.name}</h5>
                                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${group.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                                    ...
                                                    <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-gray-500">
                                                        ...
                                                        >
                                                            ...
                                                            <svg
                                                                className={`w-4 h-4 transform transition-transform ${expandedGroups.has(group._id) ? 'rotate-180' : ''}`}
                                                                fill="none"
                                                                stroke="currentColor"
                                                                viewBox="0 0 24 24"
                                                            >
                                            ...
                                            <div className="border-t border-gray-100 bg-gray-50 px-4 py-4 sm:px-6">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                                    {group.rules.map((rule: any) => {
                                                        ...
                                                        return (
                                                            <div key={ruleId} className="bg-white border border-gray-200 rounded-lg p-3 min-w-0">
