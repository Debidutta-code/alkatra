...
//     return next(new AppError(`Property not found or propertycode mismatch`, 404));
//   }
//   // 2. Match Room
...
//   // const roomNumberAvailable = await find({  });
//   // Date validation
...
//     const roomAvailability = await Inventory.find({
//       hotelCode,
...
//           }
//         },
//         // total_room: { inventoryInfo.availability?.count }
...
const getRoomsByPropertyId2 = (0, catchAsync_1.catchAsync)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!propertyDetails || propertyDetails.property_code !== hotelCode) {
        return next(new appError_1.AppError(`Property not found or property code mismatch`, 404));
    }
    // 2. Match Room
    ...
    for (const roomType of roomTypes) {
        ...
        for (const date of dateList) {
            const roomAvailability = yield inventoryModel_1.Inventory.findOne({
                hotelCode,
                invTypeCode: roomType,
                'availability.startDate': date,
                'availability.count': { $gte: numberOfRooms }
