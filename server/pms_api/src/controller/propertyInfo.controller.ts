...
const generatePropertyCode = async (propertyName: string): Promise<string> => {
  ...
  while (!isUnique && attempts < maxAttempts) {
    ...
    propertyCode = `${prefix}-${randomSuffix}`;
    const existingProperty = await PropertyInfo.findOne({ property_code: propertyCode });
    if (!existingProperty) {
      isUnique = true;
    }
    attempts++;
  ...
  if (!isUnique) {
    throw new AppError('Failed to generate a unique property code after multiple attempts', 500);
  }
  return propertyCode;
};
...
const createpropertyInfo = catchAsync( async (req: Request<UserType["userId"]>, res: Response, next: NextFunction) => {
    if (req.role === "user") {
    ...
    const property_code = await generatePropertyCode(property_name);
