...
const FilterSelect = ({
  ...
  return (
    <div className="flex-1 min-w-48">
      <h5 className="text-sm font-medium text-gray-700 mb-2">{t(`Reviews.${label}`) || label}</h5>
      <div className="relative">
        <select
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none"
        >
          {options.map((opt) => (
