...
import {
  setGuestDetails,
  setDateRangeDetails,
} from "@/Redux/slices/hotelcard.slice";
import {
  Calendar,
  Search,
  Bed,
...
interface Room {
  ...
  room_size: number;
  room_type: string;
  // Optional fields that might still be used
  image?: string[];
  amenities?: string[];
  description?: string;
  total_room?: number;
...
const RoomsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setIsLoading(true);
      try {
        // Updated to POST request with dates and hotelCode
    ...
    fetchData();
  }, [propertyId, checkInDate, checkOutDate]); // Added dependencies
  ...
  const convertAmenities = (room: Room) => {
    ...
    return {
      ...
      // Add default_image_url if needed
      default_image_url: room.image?.[0] || ""
    } as any;
  };

  const handleBookNow = (room: Room) => {
    if (!room.has_valid_rate) return; // Disable booking if no valid rate
                      ...
                      {propertyDetails?.property_contact && (
                        <div className="flex items-center text-description">
                          <svg
                            ...
                            className="h-4 w-4 text-tripswift-blue mr-2"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-between items-center mb-5">
          ...
          <div className="text-sm font-tripswift-medium text-tripswift-black/70 bg-tripswift-blue/5 px-3.5 py-1.5 rounded-lg">
            {i18next.language === "hi" && (
              <span>
                <span className="mr-1">{filteredRooms.length}</span>
            ...
            {i18next.language === "ar" && (
              <span>
                عرض {filteredRooms.length}{" "}
                {filteredRooms.length > 1 ? "غرف" : "غرفة"}
              </span>
            )}
          </div>
        </div>
