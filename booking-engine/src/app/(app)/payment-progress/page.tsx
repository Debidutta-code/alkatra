...
const STATIC_ADDRESS = "0x6A6B94f383D75D96FD3Ae508623590754e234492";
            ...
            <div className="min-h-screen bg-gradient-to-br from-[#F0F4F8] to-[#EAF2F8] flex items-center justify-center p-4 font-noto-sans">
                <div className="bg-tripswift-off-white rounded-xl shadow-lg p-6 flex flex-col items-center space-y-4 max-w-sm w-full">
                    <div className="relative">
                        <div className="w-16 h-16 border-[3px] border-tripswift-blue/20 border-t-tripswift-blue rounded-full animate-spin"></div>
                        <div className="absolute inset-1.5 w-13 h-13 border-[3px] border-transparent border-r-tripswift-blue/40 rounded-full animate-spin"
                                            ...
                                            <div className="text-center">
                                                ...
                                                <div className="bg-tripswift-off-white rounded-xl shadow-md overflow-hidden sticky top-6">
                                                    <div className="p-4 space-y-4">
                                                        ...
                                                        <div className="text-center">
                                                            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${timeElapsed >= 2400 ? 'bg-red-50 border-red-200 text-red-600' : getStatusConfig(paymentData.status).bgColor} ${timeElapsed >= 2400 ? 'border-red-200' : getStatusConfig(paymentData.status).borderColor} border-2`}>
                                                                {timeElapsed >= 2400 ? (
                                                                    <AlertCircle className="w-4 h-4 text-red-600" />
                                                                ) : (
                                                                    getStatusConfig(paymentData.status).icon
                                                                )}
                                                                <span className={`font-tripswift-bold ${timeElapsed >= 2400 ? 'text-red-600' : getStatusConfig(paymentData.status).color}`}>
                                                                    {timeElapsed >= 2400 ? 'Payment Expired' : getStatusConfig(paymentData.status).text}
                                                                </span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="text-center">
                                                            <div className={`text-xl font-tripswift-bold ${timeElapsed >= 2400 ? 'text-red-600' : 'text-tripswift-black'}`}>
                                                                {formatTime(timeElapsed)}
                                                                {timeElapsed >= 2400 && (
                                                                    <span className="text-xs block mt-1">(Session expired)</span>
                                                                )}
                                                                {timeElapsed >= 2300 && timeElapsed < 2400 && (
                                                                    <span className="text-xs block mt-1 text-yellow-600">(Expiring soon)</span>
                                                                )}
                                                            </div>
                                                            <p className="text-xs text-tripswift-black/60">
                                                                {timeElapsed >= 2400 ? 'Maximum time reached' : 'Time elapsed'}
                                                            </p>
                                                        </div>
                                                        ...
                                                        <div className="space-y-2">
                                                            ...
                                                            <div className="flex items-center gap-1 p-2 bg-green-50 border border-green-200 rounded-lg text-sm">
                                                                ...
                                                                <span className="text-green-700 font-tripswift-medium">
                                                                    {timeElapsed >= 2400 ? 'Session expired' : 'Secure Payment'}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
